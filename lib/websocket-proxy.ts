  ...
  clientWs.on('message', async (message: Buffer) => {
    try {
      ...
      if (data.type === 'auth') {
        ...
        // If no API key provided but chatbotId is, lookup from DB
        if (!apiKey && data.chatbotId) {
          console.log(`🔐 Looking up API key for chatbot ${data.chatbotId}`)
          try {
            ...
            if (chatbot) {
              // Get key from latest version config
              const latestVersion = chatbot.versions[0]
              const config = (latestVersion?.config || {}) as any
              apiKey = config.openaiAgentSdkApiKey

              if (apiKey) {
                console.log(`✅ Found API key for chatbot ${data.chatbotId}`)
              } else {
                console.warn(`⚠️ No API key configured for chatbot ${data.chatbotId}`)
              }
            } else {
              console.warn(`⚠️ Chatbot not found: ${data.chatbotId}`)
            }
          } catch (err) {
        ...
        if (apiKey) {
          ...
          // Auth failed
          console.warn(`❌ Auth failed for client ${clientId} - No API Key found`)
          if (clientWs.readyState === WebSocket.OPEN) {
            clientWs.send(JSON.stringify({
              type: 'error',
              error: { message: 'Authentication failed. Check chatbot configuration.' }
            }))
          }
        }
