...
import { createPluginSchema, runPluginMigration, dropPluginSchema } from '@/lib/plugin-schema-utils'
...
async function test() {
    ...
    try {
        ...
        await prisma.$executeRawUnsafe(`DELETE FROM service_installations WHERE service_id IN (SELECT id FROM service_registry WHERE slug = '${testPluginSlug}')`)
        await prisma.$executeRawUnsafe(`DELETE FROM service_registry WHERE slug = '${testPluginSlug}'`)
        ...
        await prisma.serviceInstallation.create({
            data: {
                id: installationId,
                serviceId: pluginId,
                dbSchema: schemaName,
        ...
        const result: any[] = await prisma.$queryRawUnsafe(`SELECT * FROM "${schemaName}".test_table`)
        ...
        try {
            await prisma.$queryRawUnsafe(`SELECT * FROM "${schemaName}".test_table`)
