...
async function createComprehensiveDataModels() {
  const client = await pool.connect()
  
  try {
    console.log('🚀 Creating comprehensive data models with attributes and sample data...')
    ...
    // Create or get the Customer Data space
    let customerSpace
...
ORDER BY count DESC`,
    ...
    console.log(`\n🎉 Comprehensive data model setup completed successfully!`)
...
if (require.main === module) {
  createComprehensiveDataModels()
    .then(() => {
      console.log('✅ Comprehensive data model creation completed successfully')
      process.exit(0)
    })
    .catch((error) => {
      console.error('❌ Comprehensive data model creation failed:', error)
      process.exit(1)
    })
}

module.exports = { createComprehensiveDataModels }
