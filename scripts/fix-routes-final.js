...
const fixes = [
    {
        path: 'src/app/api/chatbots/[chatbotId]/cache-config/route.ts',
        content: `import { requireAuth, requireAuthWithId, requireAdmin, withErrorHandling } from '@/lib/api-middleware'
...
`
    ...
    {
        path: 'src/app/api/automation/scheduler/route.ts',
...
fixes.forEach(fix => {
    const fullPath = path.resolve(process.cwd(), fix.path);
