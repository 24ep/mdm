...
export function useAgentThread({ chatbot, chatbotId, spaceId }: UseAgentThreadOptions) {
  ...
  const loadThreads = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/openai-agent-sdk/threads?chatbotId=${chatbotId}&limit=50`)
      if (response.ok) {
  ...
  const createThread = useCallback(async (threadId: string, title?: string) => {
    ...
    try {
      const response = await fetch('/api/openai-agent-sdk/threads', {
        method: 'POST',
  ...
  const updateThreadTitle = useCallback(async (threadId: string, title: string) => {
    ...
    try {
      const response = await fetch(`/api/openai-agent-sdk/threads/${threadId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
