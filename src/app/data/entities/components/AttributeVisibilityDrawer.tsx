...
export function AttributeVisibilityDrawer(props: AttributeVisibilityDrawerProps) {
  ...
  return (
    <Drawer open={open} onOpenChange={(o) => { onOpenChange(o); if (!o) setSelectedTabId(null) }}>
      <DrawerContent className="h-screen w-[45vw] flex flex-col">
        <DrawerHeader className="border-b sticky top-0 bg-background z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Eye className="h-5 w-5" />
              <DrawerTitle className="text-xl">
                {recordLayoutSettings?.customTabs?.find((t: any) => t.id === selectedTabId)?.name || 'Attributes'}
              </DrawerTitle>
            </div>
            <div className="flex items-center gap-2">
