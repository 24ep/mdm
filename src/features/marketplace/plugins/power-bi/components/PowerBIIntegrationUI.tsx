...
interface PowerBIIntegrationUIProps {
  installationId?: string
  config?: Record<string, any>
  onConfigUpdate?: (config: Record<string, any>) => void
}

export function PowerBIIntegrationUI({
  installationId,
  config = {},
  onConfigUpdate,
}: PowerBIIntegrationUIProps) {
  ...
  const handleSave = async () => {
    if (!tenantId || !clientId) {
      setTestResult({ success: false, message: 'Tenant ID and Client ID are required' })
      return
    }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure your Power BI integration settings
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <Button onClick={handleSave} disabled={!tenantId || !clientId}>
              Save Configuration
            </Button>
            <Button
              variant="outline"
